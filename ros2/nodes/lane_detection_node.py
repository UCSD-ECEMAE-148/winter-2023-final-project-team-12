...
class LaneDetection(Node):
    def __init__(self):
        ...
        self.hsv_subscriber = self.create_subscription(Int32MultiArray, HSV_TOPIC_NAME, self.update_hsv,10)
    ...
    def update_hsv(self, data):
        self.Hue_low = data.data[0]
        self.Saturation_low = data.data[1]
        self.Value_low = data.data[2]
        self.Hue_high = data.data[3]
        self.Saturation_high = data.data[4]
        self.Value_high = data.data[5]
    ...
    def locate_centroid(self, data):
        ...
        try:
            ...
            if len(cx_list) > 1:
                ...
                # calculate errors for driving between lines
                # roughly corrects parallel lines converging
                # max correction is bottom corners which are shifted WARP_Y * 400 * WARP_X
                WARP_Y = 0.05
                WARP_X = 1.0
                for i in range(len(cx_list)):
                    cx_pos = cx_list[i]
                    cy_pos = cy_list[i]
                    # value between [-1, 1] determining where cx is in image, relative to middle
                    cx_relative_image = (self.image_width-cx_pos*2.0)/self.image_width
                    # corrected location of cx is dependent on cy distance from 0, and cx distance from center line
                    cx_corrected = cx_pos + cy_pos*WARP_Y*cx_relative_image*WARP_X
                    error = float((cx_corrected-cam_center_line_x) / cam_center_line_x)
                ...
                if abs(p_horizon_diff) <= self.error_threshold:
                    ...
                    if self.get_parameter('log').value:
